...
export default {
  type: 'group',
  async calcScore(ctx: Context, definition: Definition, opts: any) {
    const { pluginInstanceMap } = ctx

    const results = await Promise.all(
      definition.config.rules.map(async rule => {
        const plugin = pluginInstanceMap[rule.definition.type]
        const result = await plugin.calcScore(ctx, rule.definition, opts)
        return {
          name: rule.name,
          type: plugin.type,
